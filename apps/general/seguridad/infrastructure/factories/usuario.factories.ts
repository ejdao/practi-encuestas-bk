...
import { FetchRolRes, FetchUsuarioRes } from '@gen/seguridad/application/responses';
...
export const usuarioOrmToUsuarioResFactory = (data: UsuarioOrm): FetchUsuarioRes => {
  ...
  rol.id = RSA_SERVICES.encryptId(data.rol.id);
  rol.codigo = data.rol.codigo;
  rol.nombre = data.rol.nombre;

  usuario.id = RSA_SERVICES.encryptId(data.id);
  ...
  usuario.nombreCompleto = data.getNombreCompleto();
  usuario.numeroCelular = data.numeroCelular;
